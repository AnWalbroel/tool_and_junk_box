...
elif len(sys.argv) == 2 and os.path.exists(os.path.dirname(sys.argv[1])):
	path_output = sys.argv[1]
else:
	raise RuntimeError("Path to be monitored could not be found.")
...
time_thres = dt.timedelta(minutes=5)	# threshold: if exceeded -> give warning
...
while all_okay:
	...
	files = sorted(glob.glob(path_output + "*"))
	filenames = [file.replace(path_output, '') for file in files]
	n_files_new = len(files)

	# update dict_files_new:
	for file, filename in zip(files, filenames):
		dict_files_new[filename] = os.stat(file).st_size
